...
class UnsortedFreqList(FreqList):
    ...
    def add(self, item):
        ...
        # Item not found, create a new node and add at the head
    ...
    def add(self, item):
        ...
        while current is not None:
            ...
            if current.next_node is None:
                # Reached last node, add new node here
...
class SortedFreqList(FreqList):
    ...
    def add(self, item):
        ...
        # ---start student section---
        if self.head is None:
          # Empty list, create first node at head
          self.head = FreqNode(item)
          return
        ...
        # Search for the node with the item
        while current is not None:
            if current.item == item:
                # Increment frequency
                current.increment()
                ...
                # Disconnect current node before reinserting
                current.next_node = None

                # If list is empty now (head is None), insert at head
                if self.head is None:
                    self.head = current
                else:
                    # Reinsert current node in sorted order
        ...
        # Item not found, add new node at end
        new_node = FreqNode(item)

        # If list is empty, just assign
        if self.head is None:
            self.head = new_node
            return
