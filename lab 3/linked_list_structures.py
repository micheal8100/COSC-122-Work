...
class Stack:
    ...
    def push(self, item):
        ...
        new_node = Node(item)            # create node with item only
        new_node.next_node = self.head   # link new node's next_node to current head
        self.head = new_node              
        # ===end student section===
    ...
    def pop(self):
        ...
        item = self.head.item      # get the item from the top node
        self.head = self.head.next_node # move head pointer down one node
        return item
